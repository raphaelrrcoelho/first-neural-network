...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Initialize weights
        self.weights_input_to_hidden = np.random.normal(0.0, scale = self.input_nodes**-0.5, 
                                                        size = (self.input_nodes, self.hidden_nodes))

        self.weights_hidden_to_output = np.random.normal(0.0, scale = self.hidden_nodes**-0.5, 
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        output_error_term = error * 1 # 1 being the f' of f(x) = x
